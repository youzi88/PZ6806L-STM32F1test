...
void FLASH_Test(void)
{
	...
	EN25QXX_Init();
	FRONT_COLOR=YELLOW;
	LCD_ShowFont12Char(10,10,"外部FLASH测试");
	LCD_ShowPicture(tftlcd_data.width-RETURN_X,tftlcd_data.height-RETURN_Y,RETURN_X,RETURN_Y,(u8 *)gImage_return);
	LCD_ShowFont12Char(10,58,"K_up: flash write..");
	LCD_ShowFont12Char(10,78,"K_down: flash read..");
	...
	{	
		...
		if(key==KEY_UP)
		{
			EN25QXX_Write(wsbuf,0,16);
			LCD_ShowFont12Char(10,98,"www.prechin.cn");
			LCD_ShowFont12Char(10,118,"Flash Write Success");
		}
		if(key==KEY_DOWN)
		{
			EN25QXX_Read(rsbuf,0,16);
			LCD_ShowFont12Char(10,158,rsbuf);	
		}
